...
`main` is no longer the original near-empty repository baseline. It now contains reconstructed control-plane source and subsequent governed control/source work.

The earlier reconstruction lineage remains useful historical provenance:

- branch: `consolidation/lineage-reconstruction-20260903`
- reconstruction commit: `b77db83794d8a34b4c434cc2f389d86f759bb9c8`

That reconstruction deliberately joined previously divergent useful lineages without replaying stale branches onto one another:
...
Those source branches remain historical provenance. Their existence or age does not make them current control.

Current release/source candidates live under `project-instructions/` and carry their own explicit status, source bindings, qualification requirements, and non-effects. Read those release-specific artifacts rather than inferring installation or runtime state from repository presence or README prose.
...
- `project-instructions/` — release/source candidates and release-bound control artifacts; each artifact's declared status governs what may be inferred from it.
