...
2. Resolve the **current recovery-surface inventory** before discovery. The inventory is bound to this exact restore-hardening owner generation plus the exact current Bus topology subject. If either owner is unavailable, conflicted, or no longer current, completion remains unresolved.
...
7. Reestablish `RELATIONAL_IDENTITY` from an inspectable private relational readback. The canonical relationship proposition digest, privacy scope, source record key/readback identity, and readback digest must agree. The `RELATIONSHIP_IDENTITY` probe expectation must be bound to that canonical digest; a recovery candidate cannot define a weaker generic label such as `counterpart` and then pass by matching itself.
